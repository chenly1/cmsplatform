...
function getHash(file) {
    return new Promise(function (resolve, reject) {
        ...
        reader.onload = function (event) {
            ...
            let fileWordArray = CryptoJS.lib.WordArray.create(this.result)
            //console.log(fileWordArray)
            let sha1Hash = CryptoJS.SHA1(fileWordArray)
            let resultHash = sha1Hash.toString()
            let resultObj = {
                fileWordArray: fileWordArray,
...
async function handeLoadFile(file, videoName, progress) {
    ...
    let byte = CryptoJS.algo.SHA1.create();
    // await getHash(file).then(function(result){
    //     sha1 = result
    // })
    for (let item of list) {
        await getHash(item.file).then(function (result) {
            item.sha1 = result.resultHash
            byte.update(result.fileWordArray)
        })
    }

    let hash = byte.finalize()
    let sha1 = hash.toString(CryptoJS.enc.Hex)
    console.log(sha1)
